...
func NewTodoListRedis(ctx context.Context, redisClient *redis.Client) *TodoListRedis {
...
func (r *TodoListRedis) HGet(userId, listId int) (string, error) {
	if listId < 0 {
		val, err := r.redisClient.HGet(r.ctx, fmt.Sprintf("user:%d", userId), "lists").Result() // Запрос значения из кэша по ключу userId и полю lists
		return val, err
	} else {
		val, err := r.redisClient.HGet(r.ctx, fmt.Sprintf("user:%d", userId), fmt.Sprintf("list:%d", listId)).Result()
		return val, err
	}
}
...
func (r *TodoListRedis) HSet(userId, listId int, data string) error {
	...
	if listId < 0 {
		pipe.HSetNX(r.ctx, fmt.Sprintf("user:%d", userId), "lists", data) // Кешируем lists в Redis
	} else {
		pipe.HSetNX(r.ctx, fmt.Sprintf("user:%d", userId), fmt.Sprintf("list:%d", listId), data) // Кешируем list в Redis
	}
...
func (r *TodoListRedis) HDelete(userId int) error {
	err := r.redisClient.HDel(r.ctx, fmt.Sprintf("user:%d", userId), "lists").Err()
	return err
}
